...
class AgentOrchestrator:
    ...
    async def _run_agent(self, agent, state: TradingAgentState, fallback_name: str = "unknown") -> TradingAgentState:
        ...
        data_result = state["agent_outputs"].get("DataWorker", {})
    ...
    async def _run_data(self, state: TradingAgentState) -> TradingAgentState:
        return await self._run_agent(self.data_agent, state, "DataWorker")

    async def _run_news(self, state: TradingAgentState) -> TradingAgentState:
        return await self._run_agent(self.news_agent, state, "NewsWorker")

    async def _run_trend(self, state: TradingAgentState) -> TradingAgentState:
        return await self._run_agent(self.trend_agent, state, "TrendWorker")

    async def _run_risk(self, state: TradingAgentState) -> TradingAgentState:
        return await self._run_agent(self.risk_agent, state, "RiskWorker")

    async def _run_ml(self, state: TradingAgentState) -> TradingAgentState:
        return await self._run_agent(self.ml_agent, state, "MLWorker")

    async def _run_synthesis(self, state: TradingAgentState) -> TradingAgentState:
        state = await self._run_agent(self.synthesis_agent, state, "synthesis_agent")
        state["final_result"] = state["agent_outputs"].get(self.synthesis_agent.name if self.synthesis_agent else "synthesis_agent", {})
        return state
