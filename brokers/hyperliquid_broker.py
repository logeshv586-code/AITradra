...
class HyperliquidBroker(BaseBroker):
    ...
    def __init__(self, private_key: Optional[str] = None, vault_address: Optional[str] = None):
        ...
        if not self.private_key:
            logger.warning("Hyperliquid private key not provided. Broker will operate in observation mode only.")
            self.exchange = None
        else:
    ...
    async def get_positions(self) -> List[Dict]:
        ...
        address = self.vault_address or (self.account.address if hasattr(self, 'account') else None)
        if not address:
            return []
    ...
    async def place_order(self, order: Order) -> Dict:
        ...
        if self.paper:
            logger.info(f"[PAPER] Hyperliquid Order: {order.side.value} {order.qty} {order.ticker}")
    ...
    async def get_candles(self, ticker: str, interval: str = "5m", limit: int = 100) -> List[Dict]:
        """Fetch historical candle data for TA indicators."""
        try:
            # Map interval if needed (HL uses 1m, 5m, 15m, 1h, 4h, 1d)
            # HL API: get_l2_book or candle data
            # info.l2_snapshot or info.candle_snapshot
            
            # The SDK info.candles method:
            candles = await asyncio.to_thread(self.info.candles_snapshot, ticker, interval, 0, 0) # 0, 0 for recent
