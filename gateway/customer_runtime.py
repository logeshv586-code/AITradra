...
class CustomerRuntimeStore:
    """Encrypted local connection registry + lightweight customer history."""
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_history_ticker ON history(ticker, created_at DESC)"
            )
        ...
        *,
        ...
        with self._connect() as conn:
            existing = conn.execute(
                "SELECT created_at, secret_blob FROM connections WHERE id = ?",
                (connection_id,),
            ).fetchone()
            ...
            secret_blob = (
                ...
                if secrets
                else (existing["secret_blob"] if existing else self._encrypt({}))
            )
            conn.execute(
                """
    ...
    def get_connection(
        self, connection_id: str, include_secrets: bool = False
    ) -> dict[str, Any] | None:
        with self._connect() as conn:
            row = conn.execute(
    ...
    def _public_connection(self, row: sqlite3.Row) -> dict[str, Any]:
        ...
        return {
            ...
            "config": config,
            "has_credentials": bool(secret_values),
        ...
        *,
        ...
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO history (
                    id, user_id, event_type, ticker, title, details_json, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    event_id,
                    DEFAULT_HISTORY_USER,
                    event_type,
