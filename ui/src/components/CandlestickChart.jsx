...
export default function AdvancedCandlestickChart({ data }) {
  ...
  return (
    <div className="relative select-none rounded-xl overflow-hidden backdrop-blur-md"
      style={{ background: T.glass, border: `1px solid ${T.border}`, boxShadow: `inset 0 1px 0 ${T.borderHl}` }}>
      {hover !== null && (
        <div className="absolute top-3 left-3 z-20 rounded-lg px-2.5 py-1.5 text-[10px] font-mono flex gap-3 shadow-2xl backdrop-blur-lg"
          style={{ background: 'rgba(10,15,25,0.8)', border: `1px solid ${T.borderHl}`, color: T.text }}>
          ...
          <span style={{ color: D[hover].c >= D[hover].o ? T.buy : T.sell, textShadow:`0 0 8px ${D[hover].c >= D[hover].o ? T.buy : T.sell}80` }}>C:{D[hover].c.toFixed(2)}</span>
      ...
      <div className="absolute top-3 right-3 flex gap-3 text-[10px] font-mono z-10">
        <span className="flex items-center gap-1.5"><span className="w-3 h-0.5" style={{ background: T.warn, boxShadow:`0 0 5px ${T.warn}` }}></span><span style={{ color: T.muted }}>MA20</span></span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-0.5" style={{ background: T.ai, boxShadow:`0 0 5px ${T.ai}` }}></span><span style={{ color: T.muted }}>MA50</span></span>
      </div>
      <svg viewBox={`0 0 ${W} ${TH + 4}`} className="w-full" style={{ height: 240 }}
        ...
        <path d={pathMA(ma20, 19)} stroke={T.warn} strokeWidth={1.5} fill="none" style={{ filter: `drop-shadow(0 0 4px ${T.warn}80)` }} />
        <path d={pathMA(ma50, 49)} stroke={T.ai} strokeWidth={1.5} fill="none" style={{ filter: `drop-shadow(0 0 4px ${T.ai}80)` }} />
        {D.map((d, i) => {
          const isUp = d.c >= d.o;
          ...
          return (
            <g key={i} opacity={hover === null || isHov ? 1 : 0.4} className="transition-opacity">
              ...
              <line x1={cx} y1={py(d.h)} x2={cx} y2={py(d.l)} stroke={col} strokeWidth={1.2} style={{ filter: isHov ? `drop-shadow(0 0 3px ${col}80)` : 'none' }} />
              <rect x={cx - candleW/2} y={bodyTop} width={candleW} height={bodyH} fill={isUp ? col : 'transparent'} stroke={col} strokeWidth={1.2} style={{ filter: isHov ? `drop-shadow(0 0 4px ${col}80)` : 'none' }} />
